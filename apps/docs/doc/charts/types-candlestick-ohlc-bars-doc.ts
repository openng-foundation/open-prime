...
@Component({
    selector: 'types-candlestick-ohlc-bars-doc',
    standalone: true,
    imports: [AppDocSectionText, AppCode, ChartsModule],
    template: `
        <app-docsectiontext>
            <p>
                Set <i>variant="ohlc"</i> to render tick-based bars instead of filled bodies. Each bar shows a vertical line spanning the high-low range with a left tick for the open price and a right tick for the close price. This is a compact
                representation common in financial charting.
            </p>
        </app-docsectiontext>
        @defer (on viewport) {
            <div class="card">
                <div style="height: 460px">
                    <p-chart-svg>
                        <p-chart-candlestick [data]="data" categoryXField="date" openField="open" highField="high" lowField="low" closeField="close" variant="ohlc" />
                        <p-chart-x-axis />
                        <p-chart-y-axis />
                    </p-chart-svg>
                </div>
...
export class CandlestickOhlcBarsDoc {
    readonly data = [
