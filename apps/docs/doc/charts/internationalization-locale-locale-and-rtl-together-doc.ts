...
@Component({
    selector: 'internationalization-locale-locale-and-rtl-together-doc',
    standalone: true,
    imports: [AppDocSectionText, AppCode, ChartsModule],
    template: `
        <app-docsectiontext>
            <p>
                Set <i>locale</i> and <i>dir</i> on the same chart root when a market or operations view needs localized numbers, localized dates, and mirrored layout. The example below uses an Arabic financial time series so the time axis, numeric
                axis, tooltip rows, legend, and overlay placement all share the same reading direction.
            </p>
        </app-docsectiontext>
        @defer (on viewport) {
            <div class="card">
...
export class LocaleLocaleAndRtlTogetherDoc {
